...
class PosViewModel(app:Application):AndroidViewModel(app){
 ...
 fun add(p:Product)=_state.update{s->s.copy(cart=s.cart.map{if(it.product.id==p.id)it.copy(quantity=it.quantity+1)else it}+if(s.cart.none{it.product.id==p.id})CartItem(p,1)else emptyList())}
 ...
 companion object{fun factory()=object:AbstractSavedStateViewModelFactory(null,null){override fun <T:ViewModel> create(k:Class<T>,h:SavedStateHandle)=PosViewModel(getApplication())}}
}
