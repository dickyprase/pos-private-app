...
import com.kopipos.android.core.storage.PrinterStore
import com.kopipos.android.core.bluetooth.BluetoothPrinterManager
import com.kopipos.android.core.bluetooth.PrinterState
...
data class CheckoutState(val orderType: String = "DINE_IN", val customerName: String = "", val tableNumber: String = "", val payment: PaymentMethod = PaymentMethod.CASH, val receivedAmount: Int = 0, val reference: String = "", val submissionToken: String = UUID.randomUUID().toString())
data class PosState(val loggedIn: Boolean = false, val loading: Boolean = false, val error: String? = null, val products: List<Product> = emptyList(), val categories: List<Category> = emptyList(), val cart: List<CartItem> = emptyList(), val shift: Shift? = null, val orders: List<Order> = emptyList(), val page: PosPage = PosPage.CASHIER, val checkout: CheckoutState? = null, val completedOrder: Order? = null, val printerMessage: String = "Printer belum dipilih", val storeName: String = "KopiPOS") { val total get() = cart.sumOf { it.lineTotal } }

class PosViewModel(app: Application) : AndroidViewModel(app) {
    private val session = SessionStore(app); private val printerStore = PrinterStore(app); private val printer = BluetoothPrinterManager(app); private var authToken: String? = null; private val api = ApiClient { authToken }.api
    ...
    fun selectPrinter(address: String, name: String) = viewModelScope.launch { printerStore.save(address, name); _state.update { it.copy(printerMessage = "Menghubungkan...") }; _state.update { it.copy(printerMessage = when (val result = printer.connectAddress(address)) { is PrinterState.Connected -> "Terhubung — ${result.name}"; is PrinterState.Error -> result.message; else -> "Belum terhubung" }) } }
    ...
    private fun apiError(error: HttpException): String? = runCatching {
        val raw = error.response()?.errorBody()?.string() ?: return null
        val json = Gson().fromJson(raw, JsonObject::class.java)
        json.get("message")?.asString?.takeIf { it.isNotBlank() } ?: json.getAsJsonObject("errors")?.entrySet()?.firstOrNull()?.value?.asJsonArray?.firstOrNull()?.asString
    }.getOrNull()
    private fun fail(message: String?) = _state.update { it.copy(loading = false, error = message ?: "Request gagal") }
}
fun formatMoney(value: Int) = String.format("%,d", value).replace(',', '.')
fun rupiah(value: Int) = "Rp " + formatMoney(value)
