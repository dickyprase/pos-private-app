...
class PosViewModel:ViewModel(){
 ...
 private val api=Retrofit.Builder().baseUrl(com.kopipos.android.BuildConfig.BASE_URL)
  .client(OkHttpClient.Builder().addInterceptor(Interceptor { chain ->
   val request = chain.request().newBuilder().apply { authToken?.let { header("Authorization", "Bearer $it") } }.build()
 ...
 fun logout()=viewModelScope.launch{runCatching{api.logout()};authToken=null;_state.value=PosState()};private fun fail(m:String?)=_state.update{it.copy(loading=false,error=m?:"Request gagal")}
}
